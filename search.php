...
<?php
	...
	$query = "SELECT COUNT(id) AS total FROM board WHERE $_POST[search_type] LIKE '%$_POST[keyword]%'";
	//echo $query."<br>";
	...
	// 각 페이지에 대한 쿼리문
	$query = "SELECT * FROM board WHERE $_POST[search_type] LIKE '%$_POST[keyword]%' ORDER BY id DESC LIMIT $limit_idx, $page_set";  //최신 글 부터 상위에 나오게 하기위해 ORDER BY id DESC
	//echo "<br>$query";
...
<?php 		
		...
		while ($row = mysql_fetch_array($postList)) {
			...
			$comm_query = "SELECT * FROM comment WHERE board_id IN(SELECT id FROM board WHERE $_POST[search_type] LIKE '%$_POST[keyword]%')";
			//echo $comm_query;
